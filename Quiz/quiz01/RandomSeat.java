...
public class RandomSeat {
	public static void main(String[] args) {
		...
		//int[] arr = seat(10);
		//System.out.println(Arrays.toString(seat(arr));
		System.out.println(Arrays.toString(seat(10)));
	}
	
	static int[] seat(int size) {
		int[] arr = new int[size];//매개변수만큼의 배열 생성
		int index = 0;
		...
		start :while(true) {
			...
			for(int i = 0; i < index; i++) {
				if(arr[i] == ran) {
					continue start;//랜덤 숫자가 같은게 나오면 while문으로 돌아감
				}
			}
